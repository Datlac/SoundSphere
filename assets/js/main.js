...
function showMainPlaylist() {
  ...
  // Hiệu ứng: Settings trượt sang phải, Universe hiện lại
  set.style.opacity = "0";
...
// ==================== SMART SWIPE LOGIC (SAFE AREA + SLIDER FIX) ====================
let touchStartX = 0;
let touchStartY = 0;
let touchEndX = 0;
...
swipeTargets.forEach((target) => {
  ...
  target.addEventListener(
    "touchstart",
    (e) => {
      // 1. KIỂM TRA: CÓ ĐANG CHẠM VÀO THANH TRƯỢT KHÔNG? (FIX MỚI)
      // Nếu người dùng đang chạm vào thẻ input (thanh tua nhạc, volume...), ta hủy lệnh vuốt ngay
      ...
      // 3. LOGIC "VÙNG CHẾT" (SAFE AREA FIX)
      // Nếu chạm quá thấp (dính thanh Home iPhone) -> Bỏ qua
  ...
  target.addEventListener(
    "touchend",
    (e) => {
      // Nếu touchStartX bị null (do chạm vào thanh kéo hoặc vùng cấm) thì thôi
...
function handleSwipeGesture() {
  if (touchStartX === null) return; // Kiểm tra lại lần nữa
  ...
  if (distance > minSwipeDistance) {
    ...
    // Hiệu ứng nháy nút Next (nếu tìm thấy nút)
    const btn = document.querySelector(".fa-forward-step")?.parentElement;
    if (btn) {
      btn.classList.add("active");
      setTimeout(() => btn.classList.remove("active"), 200);
...
function toggleAppFullScreen() {
  // Kiểm tra xem trình duyệt có đang full screen chưa
  if (
    !document.fullscreenElement && // Chuẩn chung
    !document.webkitFullscreenElement
  ) {
    // Safari/Chrome cũ

    // NẾU CHƯA -> BẬT FULL SCREEN
    if (document.documentElement.requestFullscreen) {
      document.documentElement.requestFullscreen();
    } else if (document.documentElement.webkitRequestFullscreen) {
      document.documentElement.webkitRequestFullscreen(); // Cho Safari/Chrome cũ
    }
